...
preproc :: [Label] -> Expr -> Expr
preproc ccs (ELam x e@(ELam _ _)) = ELam x (preproc ccs e)
preproc ccs (ELam x e) = ELam x (foldr EPush (preproc ccs e) (reverse ccs))
preproc ccs (ELet (x,e1) e2) = ELet (x, preproc ccs e1) (preproc ccs e2)
preproc ccs (EApp f x) = EApp (preproc ccs f) x
preproc ccs (EPush cc e) = EPush cc (preproc (cc:ccs) e)
preproc ccs other = other
...
pushCC cc ccs
  | Just trunc <- findCC cc ccs = ccs -- was: trunc
  | otherwise                   = cc:ccs
